...
router.get('/:id/export', protect, async (req, res) => {
  try {
    ...
    worksheet.addRows([
      ...
      { field: 'Loan Type', value: form.product },
      { field: 'Source', value: form.mainSource },
      { field: 'Customer Name', value: form.customerName },
      { field: 'User Email', value: form.userId?.email || 'N/A' },
      { field: 'Lead ID', value: form.leadId },
      { field: 'Company Name', value: form.companyName },
      { field: 'Mobile', value: form.mobileNumber },
      ...
      { field: 'Agent Remarks', value: form.agentRemarks },
      { field: 'Business Type', value: form.businessType },
      { field: 'Property Type (Status)', value: form.propertyType },
      { field: 'Form Status', value: form.status },
      { field: 'Interested Status', value: form.progressStatus || 'N/A' },
      { field: 'Agent', value: form.agentName },
      { field: 'Agent ID', value: form.agentId },
      ...
      { field: 'Supervisor', value: form.supervisorName },
      { field: 'Supervisor ID', value: form.supervisorId },
      ...
      // Review Information
      { field: 'Reviewed By', value: form.reviewedBy?.name || 'N/A' },
      { field: 'Reviewed At', value: form.reviewedAt ? new Date(form.reviewedAt).toLocaleDateString() : 'N/A' },
      { field: 'Review Comment', value: form.reviewComment || 'N/A' }
    ]);
...
router.post('/:id/reminder', [
  ...
  try {
    ...
    if (form.userId.toString() !== req.user._id.toString() && req.user.role !== 'admin') {
      return res.status(403).json({
        success: false,
        message: 'Not authorized to set reminder on this form'
      });
    }
